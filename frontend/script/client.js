...
logout.addEventListener("click",()=>{
    let url="http://localhost:8600/user/logout"
...
socket.on("game",(data)=>{
    console.log(data)
        ...
        // showing whose turn is next 
        let whoisnext= document.getElementById("turn")
        if(data.nextplayer==username){
        whoisnext.innerText=`it is your turn`
        }
        else{
            let secondplayer=data.player.filter((ele)=>{
                return ele.name!==username
            })
            console.log(secondplayer,"*****")
            whoisnext.innerText=`wait for ${secondplayer[0].name}'s move....`
        }
        
        // deleteing leaderboard innnert elemets after 3rounds
        if(data.winnerdata.length==0){
            console.log("gman,gmmmmm//////")
            r1p1.innerHTML=null
            r1p2.innerHTML=null
            r2p1.innerHTML=null
            r2p2.innerHTML=null
           ...
           if(data.winnerdata.length>=3){
              ...
              if(winner=="draw"){
                ...
                let button=document.createElement("button")
                 overallwinner.append(button)
                button.innerText="restart"
                    data.winnerdata=[]
                    data.winningline=[]
                button.addEventListener("click",()=>{
                    overallwinner.innerHTML=null
                })
              }
              else{
                overallwinner.innerHTML=`over all winner is ${winner} `
                let button=document.createElement("button")
                 overallwinner.append(button)
                button.innerText="restart"
                data.winnerdata=[]
    ...
    if(isnext==true){
        let isclicked=data.clickedcell.includes(event.target.id)
